...
describe('builtin', () => {
  ...
  it('ceil', async () => {
    expect(await render('{{= x | ceil }}', { x: 1.2 })).toMatchInlineSnapshot(
      `"2"`,
    );
    expect(await render('{{= x | ceil }}', { x: '1.0' })).toMatchInlineSnapshot(
      `"1"`,
    );
  });
  ...
  it('fallback', async () => {
    ...
    expect(
      await render('{{= o | get("x") | fallback(o | get("y")) }}', {
        o: { x: 0, y: 2 },
      }),
    ).toMatchInlineSnapshot(`"0"`);
    expect(
      await render('{{= o | get("x") | fallback(o | get("y"), true) }}', {
        o: { x: 0, y: 2 },
      }),
    ).toMatchInlineSnapshot(`"2"`);
  });
