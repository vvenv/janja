...
export const getNamedAndUnnamedTags = (tags: Tag[]) =>
  tags.reduce(
    (acc, tag) => {
      if (tag.names.length) {
        for (const name of tag.names) {
          acc.namedTags[name] = [...(acc.namedTags[name] || []), tag];
        }
      } else {
        acc.unnamedTags.push(tag);
      }
      return acc;
    },
    {
      namedTags: {} as Record<string, Tag[]>,
      unnamedTags: [] as Tag[],
    },
  );

export const { namedTags, unnamedTags } = getNamedAndUnnamedTags(tags);
