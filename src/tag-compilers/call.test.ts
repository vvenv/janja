...
it('invalid', async () => {
  ...
  try {
    ...
    expect(error.details).toMatchInlineSnapshot(`
      ""call" tag must specify a "macro" name
...
it('macro and call', async () => {
  expect(
    ...
    `""use strict";return(async()=>{let s="";c.m=(x,y)=>async(_c)=>{const c_0={...c,x,y};s+=e(c_0.x);await _c?.();s+=e(c_0.y);};await c.m("f",1)(async()=>{s+="3";});return s;})();"`,
  )
  expect(
    await compile(
    ...
    `""use strict";return(async()=>{let s="";c.m=(x="f",y=1)=>async(_c)=>{const c_0={...c,x,y};s+=e(c_0.x);await _c?.();s+=e(c_0.y);};await c.m()(async()=>{s+="3";});return s;})();"`,
  )
})
